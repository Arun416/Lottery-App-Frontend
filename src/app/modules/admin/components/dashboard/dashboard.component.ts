...
export class DashboardComponent implements OnInit, OnDestroy  {
  ...
  getLotteryValues(){
    ...
    if(dataInitial!== null){
    this.lotteryInputForm.patchValue({
  ...
  getAllLotteryInfo(lotteries_List:any){
    ...
    if(localStorage.getItem('lottery_No') === null && 
     ...
     for(let i=0; i<=this.storedLotteryNumber1.length;i++){
      ...
      this.lotteryInputForm.valueChanges.subscribe((editValues:any) => {
  ...
  onSubmitLottery(formData:any){
    this.isLoading = true
    const { textbox1, textbox2, textbox3, textbox4 } = this.lotteryInputForm.value;
